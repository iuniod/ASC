...
__global__ void kernel_full_atomics(int *data, int *results) {
  __shared__ int sharedResults[INDEX_NUM];

  // init shared memory
  if (threadIdx.x == 0) {
    sharedResults[INDEX_SUM] = 0;
    sharedResults[INDEX_MAX] = 0;
    sharedResults[INDEX_MIN] = NUM_MAX;
  }
  ...
  atomicAdd(&sharedResults[INDEX_SUM], data[index]);
  atomicMax(&sharedResults[INDEX_MAX], data[index]);
  atomicMin(&sharedResults[INDEX_MIN], data[index]);

  __syncthreads();

  // write back
  if (threadIdx.x == 0) {
    atomicAdd(&results[INDEX_SUM], sharedResults[INDEX_SUM]);
    atomicMax(&results[INDEX_MAX], sharedResults[INDEX_MAX]);
    atomicMin(&results[INDEX_MIN], sharedResults[INDEX_MIN]);
  }
}
