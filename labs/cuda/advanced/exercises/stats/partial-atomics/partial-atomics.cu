...
__global__ void kernel_partial_atomics(int *data, int *results) {
  __shared__ int partialResults[3 * BLOCK_SIZE];

  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  int localId = threadIdx.x;

  // init
  partialResults[localId] = 0;
  partialResults[localId + BLOCK_SIZE] = 0;
  partialResults[localId + 2 * BLOCK_SIZE] = NUM_MAX;

  // compute partial results
  for (int i = tid; i < ITEMS_NUM; i += blockDim.x * gridDim.x) {
    partialResults[localId] += data[i];
    partialResults[localId + BLOCK_SIZE] =
        (data[i] > partialResults[localId + BLOCK_SIZE]) ? data[i]
                                                         : partialResults[localId + BLOCK_SIZE];
    partialResults[localId + 2 * BLOCK_SIZE] =
        (data[i] < partialResults[localId + 2 * BLOCK_SIZE]) ? data[i]
                                                            : partialResults[localId + 2 * BLOCK_SIZE];
  }
  ...
  if (localId == 0) {
    for (int i = 0; i < BLOCK_SIZE; i++) {
      atomicAdd(&results[INDEX_SUM], partialResults[i]);
      atomicMax(&results[INDEX_MAX], partialResults[i + BLOCK_SIZE]);
      atomicMin(&results[INDEX_MIN], partialResults[i + 2 * BLOCK_SIZE]);
    }
  }
}
