...
int main(void) {
  ...
  for (int i = 0; i < BUF_32M; i += BUF_2M) {
    cudaMemcpy(device_array_c, device_array_a + i, BUF_2M * sizeof(int),
               cudaMemcpyDeviceToDevice);
    cudaMemcpy(device_array_a + i, device_array_b + i, BUF_2M * sizeof(int),
               cudaMemcpyDeviceToDevice);
    cudaMemcpy(device_array_b + i, device_array_c, BUF_2M * sizeof(int),
               cudaMemcpyDeviceToDevice);
  }
